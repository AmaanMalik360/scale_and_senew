...
const CartItemRow = ({ item }: CartItemRowProps) => {
  ...
  const handleUpdateQuantity = (newQuantity: number) => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      updateCartItem({ productId: item.productId, quantity: latestQuantityRef.current }).catch(() => {});
    }, DEBOUNCE_MS);
  };
    ...
    <div className="flex gap-4">
      <div className="w-20 h-20 bg-muted/10 rounded-lg overflow-hidden relative">
        {item.image ? (
          ...
          <div className="w-full h-full rounded-lg bg-[var(--admin-border-light)] overflow-hidden shrink-0 flex items-center justify-center">
